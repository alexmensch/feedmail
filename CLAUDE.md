...
src/
  ...
  templates/              # Handlebars (.hbs) source files, precompiled at build time
    partials/
      ...
      admin-layout.hbs    # Authenticated page layout: top nav bar, HTMX script, content via {{> @partial-block}}
      ...
      public-page-layout.hbs  # Shared layout for public pages (verify, unsubscribe, error) — CDN Pico CSS
...
assets/
  admin/
    pico.min.css          # Pico CSS v2.1.1 — default variant, committed as-is, no build pipeline
    styles.css            # Pico CSS overrides: feedmail-specific components only (feedback, feed rows, input suffix, etc.)
...
- **Pico CSS styling:** Admin console and public-facing pages use Pico CSS v2 (default variant) for semantic HTML styling — elements like `<nav>`, `<article>`, `<table>`, and form elements are styled by Pico's defaults with no custom classes needed. Pico CSS is committed as a static asset (`assets/admin/pico.min.css`) alongside HTMX. A minimal override stylesheet (`assets/admin/styles.css`) contains only feedmail-specific component styles (feedback messages, feed rows, input suffix, button variants, loading indicators, etc.) — all colors use Pico custom properties (`--pico-*`) for automatic dark mode support. Public-facing pages (verify, unsubscribe, error) load Pico from CDN via a shared `public-page-layout.hbs` partial. Layout partials (`admin-layout.hbs` for authenticated pages with horizontal top nav, `admin-auth-layout.hbs` for centered auth pages) use Handlebars partial blocks (`{{> @partial-block}}`).
- **Admin console static assets:** The Admin Worker serves static files from the `assets/` directory via Cloudflare Workers static assets (`[assets]` config in `wrangler.admin.toml`). Currently contains `admin/pico.min.css`, `admin/styles.css`, and `admin/htmx.min.js`. The `assets/` directory is excluded from ESLint.
