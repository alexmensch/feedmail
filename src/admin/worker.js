...
/**
 * Route a request to its handler and return the Response (before security
 * headers are applied). Trailing-slash normalization has already run, so
 * `url.pathname` is canonical.
...
export default {
  /**
   * HTTP request handler. Normalizes trailing slashes, routes the request, and
   * applies the admin security headers to every response.
   * @param {Request} request
   * @param {object} env
   * @returns {Promise<Response>}
   */
